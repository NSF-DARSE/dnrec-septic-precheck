...
# How many symbols refine at once. Their passes are independent, so this is width
# against a fixed depth of MAX_REFINE_PASSES: the read stops costing symbols times
# passes round trips and costs passes round trips instead.
#
# Six rather than unbounded because the account is shared and Bedrock throttles. A
# sheet with a dozen symbols opening a dozen concurrent conversations trades a slow
# read for a failed one.
REFINE_CONCURRENCY = 6
...
class SymbolLocator:
    ...
    def locate(
        ...
        if refine and symbols:
            # One symbol at a time was the whole cost of a read. Refinement is up to
            # MAX_REFINE_PASSES model calls per symbol and each call is a round trip
            # of a second or two, so five symbols ran twenty calls end to end and a
            # sheet took around a minute.
            #
            # Symbols do not depend on each other: each refines inside its own crop
            # of the same page. So the passes for one symbol stay sequential, because
            # each pass crops around the previous answer, and the symbols themselves
            # run side by side. Depth stays at MAX_REFINE_PASSES while width
            # collapses, which is where the minute went.
            #
            # Bounded rather than unbounded: this is a shared account with a
            # throttling quota, and a sheet with a dozen symbols firing a dozen
            # concurrent conversations is how a read fails with ThrottlingException
            # instead of taking too long.
            from concurrent.futures import ThreadPoolExecutor

            def _one(symbol):
                return symbol, _refine_iteratively(
            ...
            workers = min(REFINE_CONCURRENCY, len(symbols))
            with ThreadPoolExecutor(max_workers=workers,
                                    thread_name_prefix="refine") as pool:
                for symbol, (passes, notes) in pool.map(_one, symbols):
