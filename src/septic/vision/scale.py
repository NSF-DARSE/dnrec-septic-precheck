...
def measure_bar_extent(image_path: Path, approx: PixelBox,
    ...
    Returns (left_x, right_x, length, top_y, band_height, method) in full page
    pixels.
    ...
    return left_x, right_x, length, top_y, band_height, "ink_projection_longest_run"
                    ...
                    if span_feet and span_feet > 0:
                        try:
                            (left_x, right_x, length, bar_top, bar_height,
                             method) = measure_bar_extent(path, approx)
        ...
        if bar_fpp is not None and ratio_fpp is not None:
            ...
            if agreement < 85.0:
                warnings.append(
                    ...
                    f"written ratio gives {ratio_fpp:.4g}, agreeing to only "
                    f"{agreement:.0f} percent; the measured bar was used"
                )
