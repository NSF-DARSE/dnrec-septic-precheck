...
def score_page(text: str, form_fields: int = 0, lines: int = 0) -> SheetPick:
    ...
    return SheetPick(
        ...
        line_count=lines,
        # Naming itself is not enough. A drawing is dimensioned, and it carries a
        # scale. This is the flag that separates a real sheet from the synthetic
        # packet's placeholder, which says "SITE PLAN" and nothing measurable.
        looks_drawn=bool(
            dims >= DIMENSION_FLOOR
            or SCALE_NOTATION.search(text)
            or "graphic scale" in low
        ),
    )
...
def rank_pages(document: Document) -> list[SheetPick]:
    ...
    picks.sort(key=lambda p: (-p.score, p.page))
    return picks
