...
STYLE_SOURCES: dict[str, dict] = {
    "septic_tank": {
        ...
        "description": (
            ...
            "box with no circles at all, so the absence of circles does not by "
            "itself rule a box out. What does rule it out: a small solid BLACK "
            "FILLED box with no visible pipe connection and a label like TP, TPA, "
            "TPB, TP1 or TP2 is a soil test pit, not a tank -- see test_pit. A real "
            "septic tank is usually drawn as an unfilled or lightly hatched "
            "rectangle with a callout nearby naming its capacity, such as "
            "'PROPOSED 1000 GALLON ... SEPTIC TANK'. Where two boxes are drawn "
            "together connected by a pipe, the septic tank is the LARGER box; the "
            "smaller box downstream of it is a distribution box, not the tank"
        ),
    },
    ...
    "test_pit": {
        "dir": None,
        "description": (
            "a soil test pit or percolation test hole marker: a small solid "
            "black filled square or rectangle, usually labelled TP, TPA, TPB, "
            "TPC, TP1, TP2, or similar, often with a hole number nearby such as "
            "'90'. It has no pipe connecting it to anything else. It looks like a "
            "tank at a glance because both are small dark boxes, but it is never "
            "a septic tank -- see septic_tank for how to tell them apart"
        ),
    },
    "well": {
    ...
    # The three below are described from the real sheets in assets/examples rather
    # than from the regulation, because what matters is how these draughtsmen draw
    # them. Description only for now: crops can be cut from those same sheets, and
    # prose is what establishes whether the class is worth having at all.
    "disposal_area": {
        "dir": ASSETS_DIR / "disposal_style",
        "description": (
            "the disposal area, also called the absorption or OWTDS area: a group "
            "of long narrow parallel rectangles drawn side by side, which are the "
            "trenches or laterals, or a single hatched or stippled rectangle which "
            "is a sand mound or seepage bed. It is often rotated rather than square "
            "to the sheet. Labels beside it include SEPTIC FEASIBILITY AREA, OWTDS "
            "AREA, DISPOSAL AREA, or a note such as PROPOSED 6 TRENCHES 3'W X 60'L. "
            "Return the whole group of parallel trenches as ONE disposal_area, not "
            "one per trench. A separate region labelled SPARE AREA or SPARE SEPTIC "
            "AREA is the reserve area and is a different object, so do not merge it "
            "with the primary one"
        ),
    },
    "watercourse": {
        "dir": ASSETS_DIR / "watercourse_style",
        "description": (
            "a watercourse: a ditch, stream, creek, swale or the edge of a pond or "
            "impoundment. Drawn as a long meandering line, sometimes a pair of "
            "lines for a channel, sometimes with a hatched or stippled band beside "
            "it for a wetland or buffer. Labels include TAX DITCH, EXISTING DITCH, "
            "STREAM, WETLANDS, or WETLANDS BUFFER. It runs across the sheet rather "
            "than being a compact symbol, so return the portion nearest the "
            "disposal area"
        ),
    },
    "escarpment": {
        "dir": None,
        "description": (
            "an escarpment or top of bank: the break in slope where ground falls "
            "away toward a ditch, stream or pond. Drawn as a line where contour "
            "lines bunch closely together, or as a hachured or ticked line with "
            "short marks on the downhill side, and labelled TOP OF BANK, TOB, "
            "ESCARPMENT or BREAK IN SLOPE. It is a boundary rather than a compact "
            "symbol, so return the segment nearest the disposal area"
        ),
    },
    "property_line": {
        "dir": None,
        "description": (
            "a property or lot line: the boundary of the parcel. Drawn as a heavy "
            "line, often dash-dot, and usually annotated along its length with a "
            "surveyed bearing and distance such as N 89 15'20\" E 271.60'. Corners "
            "may carry a small square or the marks CM, CMF or IPF for a monument. A "
            "sheet legend often names the convention explicitly as PROPERTY LINE. "
            "It is a boundary, not a compact symbol, so return the segment nearest "
            "the disposal area"
        ),
    },
}
